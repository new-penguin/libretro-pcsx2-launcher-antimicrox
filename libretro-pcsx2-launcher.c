...
bool retro_load_game(const struct retro_game_info *info)
{
   ...
   char command[512] = "~/.config/retroarch/system/antimicrox.AppImage --tray || flatpak run io.github.antimicrox.antimicrox --tray || antimicrox --tray & pcsx2-qt -fullscreen";
   ...
   strcpy(command, "flatpak run net.pcsx2.PCSX2 -fullscreen");
   if (info != NULL && info->path != NULL && info->path[0] != '\0') {
      // Execute with --batch.
      sprintf(command, "%s \"%s\"", command, info->path);
   }
   ...
   strcpy(command, "~/.config/retroarch/system/pcsx2.AppImage -fullscreen");
   if (info != NULL && info->path != NULL && info->path[0] != '\0') {
      // Execute with --batch.
      sprintf(command, "%s \"%s\"", command, info->path);
   }
